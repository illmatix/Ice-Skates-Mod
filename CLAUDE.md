# Ice Skates Mod — Project Instructions
...
## Build & Test
- **Build:** `dotnet build` (auto-deploys DLL + assets to `~/.config/VintagestoryData/Mods/IceSkates/`)
- **Quick test:** Drop folder with `.cs` files into `Mods/` — Vintage Story compiles at runtime
- **SDK:** .NET 8.0, pinned via `global.json`

## Project Structure
- `src/` — C# source files
- `assets/iceskates/` — Item/block types, recipes, shapes, lang
- `docs/` — Detailed reference documentation
- `.run/` — Rider run configs (gitignored)

## Key Source Files
- `IceSkatesMod.cs` — Mod entry point, config loading, recipe pruning
- `ItemIceSkates.cs` — Lining application, whetstone sharpening, tooltips
- `EntityBehaviorIceSkating.cs` — Ice detection, speed/hunger stat application, particles
- `IceSkatesConfig.cs` — Tier-grouped config with master toggles + per-metal switches
- `ItemIceRinkCreator.cs` — Creative-mode rink generator
- `BlockSharpenerJig.cs` / `BlockEntitySharpenerJig.cs` — Jig block

## Conventions
- Target Vintage Story 1.21.6+
- Lining stored as tree attributes on itemstack, not variants
- Foot slot accessed via `EnumCharacterDressType.Foot` enum — never hardcode slot index
